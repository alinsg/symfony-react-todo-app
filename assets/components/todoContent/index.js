    ...
    return (
        ...
        >
            ...
            <Flex
                ...
                overflowY={"scroll"}
            >
                {context.todos.map(todo => <ContentItem key={todo.task + todo.id} text={todo.task} id={todo.id}/>)}
                <Flex
                    justifyContent={"flex-start"}
                    padding={"8px 16px"}
                    flexDirection={"column"}
                >
                    {isAddingTodo ? <Input placeholder="Add a task" value={newTodo} onChange={handleNewTodo}/> : <></>}
